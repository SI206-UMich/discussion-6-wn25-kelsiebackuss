...
def load_csv(f):
    ...
    with open(full_path, mode='r', newline='') as file: 
        ...
        headers = next(reader)  # First row contains column headers: Year, JAN, FEB, ..., DEC
        
        for row in reader:
            year = row[0]  # First column is the year
            data[year] = {headers[i]: row[i] for i in range(1, len(row))}  # Month: Value
